...
struct Textdraw {
	...
	float position[2];
	uint8_t unk2[8];
	uint32_t field_99B;	// -1 by default
	uint32_t field_99F;	// -1 by default
	uint32_t index;		// -1 if bad
	uint8_t field_9A7;	// = 1; 0 by default
	uint16_t model;
	float rotation[3];
	float zoom;
	uint16_t color[2];
...
struct RemotePlayersInfo {
	struct RemotePlayer* remotePlayer[SAMP_MAX_PLAYERS];
...
struct SAMPFUNCS_API PlayerPool {
	...
	RemotePlayersInfo remotePlayersInfo;
...
struct SAMPFUNCS_API LocalPlayer {
	...
	uint16_t unk2;
	uint32_t lastUpdate;
	uint32_t lastSpectateTick;
	uint32_t lastAimTick;
	uint32_t lastStatsTick;
	struct CameraTarget {
		uint16_t objectId;
		uint16_t vehicleId;
		uint16_t playerId;
	...
	uint32_t spawnSelectionStartTick;
	struct SpawnInfo {
		uint8_t teamId;
		int skinId;
		uint8_t unk;
